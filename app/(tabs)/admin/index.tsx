...
const CrearPollaSCreen = () => {
  ...
  const updateCarrera = (date: Date) => {
    switch (carrera) {
      case '1':
        setPolla({
          ...polla, carrera1_dist: parseInt(distancia), carrera1_ejem: parseInt(ejemplares), carrera1_hor: date.toLocaleTimeString('es-ES', {
            hour: '2-digit',
            minute: '2-digit',
            hour12: true
          })
      ...
      case '2':
        setPolla({
          ...polla, carrera2_dist: parseInt(distancia), carrera2_ejem: parseInt(ejemplares), carrera2_hor: date.toLocaleTimeString('es-ES', {
            hour: '2-digit',
            minute: '2-digit',
            hour12: true
          })
        ...
        case '3':
        setPolla({
          ...polla, carrera3_dist: parseInt(distancia), carrera3_ejem: parseInt(ejemplares), carrera3_hor: date.toLocaleTimeString('es-ES', {
            hour: '2-digit',
            minute: '2-digit',
            hour12: true
          })
        ...
        case '4':
        setPolla({
          ...polla, carrera4_dist: parseInt(distancia), carrera4_ejem: parseInt(ejemplares), carrera4_hor: date.toLocaleTimeString('es-ES', {
            hour: '2-digit',
            minute: '2-digit',
            hour12: true
          })
        ...
        case '5':
        setPolla({
          ...polla, carrera5_dist: parseInt(distancia), carrera5_ejem: parseInt(ejemplares), carrera5_hor: date.toLocaleTimeString('es-ES', {
            hour: '2-digit',
            minute: '2-digit',
            hour12: true
          })
        ...
        case '6':
        setPolla({
          ...polla, carrera6_dist: parseInt(distancia), carrera6_ejem: parseInt(ejemplares), carrera6_hor: date.toLocaleTimeString('es-ES', {
            hour: '2-digit',
            minute: '2-digit',
            hour12: true
          })
      ...
      >
        <ScrollView style={{ width: '100%' }} contentContainerStyle={{ alignItems: 'center', gap: 20, paddingBottom: 20 }}>
          <Card style={{ width: '90%', justifyContent: 'flex-start', alignItems: 'flex-start', gap: 2, display: polla.polla.length > 0 ? 'flex' : 'none', flexDirection: 'column', padding: 10, marginTop: 20 }}>
            ...
            <Text style={{ fontSize: 9 }}>Hora de Cierre de la Polla : {polla.hora_de_cierre?.toLocaleTimeString('es-ES', {
              hour: '2-digit',
              minute: '2-digit',
              hour12: true
            })}</Text>
          ...
          <View style={{ borderWidth: 1, borderRadius: 8, width: '90%' }}>
            <Picker
              ...
              onValueChange={(itemValue, itemIndex) => handleSelecthipodromo(itemValue, itemIndex)}>
              <Picker.Item label="Seleccionar hipodromo" value={0} style={{ fontSize: 13 }} />
              {
                hipodromos && hipodromos.map(p => <Picker.Item label={p.name as string} value={p.name} style={{ fontSize: 14 }} />)
              }
            </Picker>
          </View>
          <View style={{ width: '90%', flexDirection: 'column', gap: 20 }}>
            <View style={{ flexDirection: 'row', gap: 5, flex: 1, alignItems: 'center', width: '80%', justifyContent: 'space-between' }}>
              <ThemeTextInput
            ...
            <View style={{ display: visible ? 'flex' : 'none', }}>
              <Text style={{ textAlign: 'center', fontWeight: '700' }}>{VALIDAS[Number(carrera)]}</Text>
              <Card style={{ flex: 1, flexDirection: 'row', gap: 3, justifyContent: 'space-between', alignItems: 'center', marginHorizontal: 0, width: '100%', margin: 'auto', backgroundColor: '#c37373', padding: 10 }}>
              ...
              <Button
                disabled={ejemplares === '0' || distancia === '0' || ejemplares.length === 0 || distancia.length === 0}
                title="Hora de Carrera" onPress={() => {
                  if (ejemplares === '0' || distancia === '0' || ejemplares.length === 0 || distancia.length === 0) return
                  handleCarrera()
                }} />
            </View>
            <View style={{ padding:4, borderWidth:0.5, borderRadius:8, width:'100%' }}>
            ...
            <View style={{ flexWrap: 'wrap', flexDirection: 'row', flex: 1, justifyContent: 'space-between', width: '100%', borderWidth: 1, borderColor: '#ccc', borderRadius: 5, padding: 10 }}>
              <Button title="1ra" onPress={() => { setCarrera('1'); setVisible(true) }} />
              <Button title="2da" onPress={() => { setCarrera('2'); setVisible(true) }} />
              <Button title="3ra" onPress={() => { setCarrera('3'); setVisible(true) }} />
            </View>
            <View style={{ flexWrap: 'wrap', flexDirection: 'row', flex: 1, justifyContent: 'space-between', width: '100%', borderWidth: 1, borderColor: '#ccc', borderRadius: 5, padding: 10 }}>

              <Button title="4ta" onPress={() => { setCarrera('4'); setVisible(true) }} />
              <Button title="5ta" onPress={() => { setCarrera('5'); setVisible(true) }} />
              <Button title="6ta" onPress={() => { setCarrera('6'); setVisible(true) }} />
            </View>
          </View>
